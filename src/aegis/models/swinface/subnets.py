...
class LANet(torch.nn.Module):
    def __init__(self, in_chans=512, reduction_ratio=2.0):
        ...
        self.conv1 = nn.Conv2d(
            self.in_chans, self.mid_chans, kernel_size=(1, 1), stride=(1, 1), bias=False
        )
        self.conv2 = nn.Conv2d(
            self.mid_chans, 1, kernel_size=(1, 1), stride=(1, 1), bias=False
        )
...
def MAD(x, p=0.6):
    ...
    mask1 = torch.cat(
        [torch.randperm(C).unsqueeze(dim=0) for j in range(B)], dim=0
    ).cuda()
    ...
    def __init__(
        ...
        if not self.conv_shared:
            if conv_mode == "normal":
                self.conv = ConvLayer(
                    in_chans=in_chans,
                    out_chans=feature_dim,
                    conv_mode="normal",
                    kernel_size=kernel_size,
                )
            elif conv_mode == "split" and in_chans == 2112:
                self.conv = ConvLayer(
                    in_chans=[192, 384, 768, 768],
...
class FeatureAttentionModule(torch.nn.Module):
    def __init__(
        ...
        spatial_attention=None,
        la_num_list=[2 for j in range(11)],
        pooling="max",
    ):
        super().__init__()
        ...
        for i in range(branch_num):
            net = FeatureAttentionNet(
                in_chans=in_chans,
                feature_dim=feature_dim,
                conv_shared=conv_shared,
                conv_mode=conv_mode,
                kernel_size=kernel_size,
                channel_attention=channel_attention,
                spatial_attention=spatial_attention,
                la_branch_num=la_num_list[i],
                pooling=pooling,
            )
