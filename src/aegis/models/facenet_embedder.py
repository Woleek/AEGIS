"""FaceNet embedder implementations for identity masking and evaluation.

This module provides two embedder classes following the same pattern as SwinFace/TransFace:
- FaceNetEmbedder: For batch evaluation with numpy arrays
- FaceNet: For single-image attack scenarios with PyTorch tensors

FaceNet paper: FaceNet: A Unified Embedding for Face Recognition and Clustering (CVPR 2015)
Architecture: Inception-ResNet-V1, input 160x160, output 512-dim L2-normalized embeddings.
"""
...
from .facenet import load_facenet_model
from ..config import MODELS_DIR

_FACE_SIZE = 160


class FaceNetTorchModel(nn.Module):
    """PyTorch wrapper for FaceNet (InceptionResnetV1) model."""
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            images: Tensor of shape [B, 3, 160, 160] with pixel values in [-1, 1].
    ...
    def preprocess_face(self, face_crop: torch.Tensor) -> torch.Tensor:
        """Preprocess face crop for FaceNet model.

        Args:
            face_crop: Tensor with pixel values in [0, 255], shape [C, H, W].

        Returns:
            Normalized tensor with values in [-1, 1], shape [C, 160, 160].
        """
...
class FaceNetEmbedder(BaseEmbedder):
    """FaceNet embedder for batch evaluation.

    Handles batch processing of images for evaluation, including face detection
    and alignment using InsightFace. Uses 160x160 face crops.
    """
    ...
    def _align_face(self, image: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Aligned face crop of size 160x160.
        """
        try:
            ...
            if kpss is not None and len(kpss) > 0:
                return insightface.utils.face_align.norm_crop(
                    image, landmark=kpss[0], image_size=_FACE_SIZE
                )
    ...
    def _prepare_tensor(self, aligned_image: np.ndarray) -> torch.Tensor:
        ...
        Args:
            aligned_image: Aligned face crop as numpy array (160x160x3).
    ...
    def embed(self, paths: Sequence[Path], key_fn) -> Dict[str, np.ndarray]:
        ...
        for chunk in tqdm(
            ...
            desc="Embedding images with FaceNet",
...
class FaceNet(FaceNetEmbedder):
    """FaceNet embedder for single-image attack scenarios.
    ...
    def _align_face(self, image: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Aligned face crop tensor of shape (C, 160, 160).
        ...
        if kpss is not None and len(kpss) > 0:
            ...
            raise ValueError("No face detected in the image for FaceNet embedding.")
        return crop
