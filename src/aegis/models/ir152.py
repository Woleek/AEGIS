"""IR-152 face recognition model for adversarial attacks.

Architecture: IResNet-152 IR mode (512-D embedding).
Reference:    DiffAM / TreB1eN InsightFace_Pytorch (assets/models/ir152.py)
Checkpoint:   models/ir152.pth
"""
...
_IR152_SPECS: dict[str, VerificationModelSpec] = {
    "r152": VerificationModelSpec(
        name="ir152",
        variant="r152",
        display_name="IR-152",
        embedding_dim=512,
        ...
        assets=(
            ModelAssetSpec(
                key="checkpoint",
                path=MODELS_DIR / "ir152.pth",
                description="IR-152 pretrained weights",
                install_hint=(
                    "Place the pretrained checkpoint at models/ir152.pth. "
                    "Compatible with DiffAM assets/models/ir152.pth."
                ),
            ),
            ModelAssetSpec(
                key="detector",
...
def get_ir152_specs() -> dict[str, VerificationModelSpec]:
    return dict(_IR152_SPECS)


def _load_ir152_backbone(checkpoint_path: Path, device: str) -> IrseBackbone:
    # drop_ratio=0.4 matches the DiffAM ir152.py Backbone(input_size, 152, 'ir') default.
    model = IrseBackbone(num_layers=152, drop_ratio=0.4, mode="ir")
    ...
    if not isinstance(state_dict, dict):
        raise ModelAssetMissingError(
            f"IR-152 checkpoint is not a state dict: {checkpoint_path}"
        )
...
class IR152:
    """IR-152 embedder for differentiable face verification.
    ...
    4. Forward through IrseBackbone (152-layer IR) → 512-D BN features.
    ...
    def __init__(
        ...
        model_path: Path,
        variant: Literal["r152"] = "r152",
    ) -> None:
        self.spec = _IR152_SPECS[variant]
        ...
        self.model: IrseBackbone = _load_ir152_backbone(
    ...
    def _align_face(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if detection is None:
            raise FaceNotDetectedError("No face detected for IR-152 embedding.")
